"""
测试 Phase 3 API 端点（使用 requests 库）
需要先启动后端服务：python main.py
"""
import requests
import json
...
def test_set_user_type():
    ...
    response = requests.post(f"{BASE_URL}/api/preferences/user-type", json={
        "user_type": "student",
        "user_id": 1
    })
    ...
    if response.status_code == 200:
        data = response.json()
        print(f"   [OK] 成功: {data['message']}")
        print(f"   user_type: {data['preferences']['user_type']}")
    else:
        print(f"   [ERROR] 失败: {response.text}")
    ...
    if response.status_code == 400:
        print(f"   [OK] 正确拒绝: {response.json()['detail']}")
    else:
        print(f"   [ERROR] 应该返回400错误")
...
def test_get_standard_profiles():
    ...
    if response.status_code == 200:
        data = response.json()
        print(f"   [OK] 成功获取 {len(data['profiles'])} 个模板")
...
def test_set_personalization_params():
    ...
    # 测试3.1: 正常参数
    print("\n[INFO] 测试3.1: 设置正常参数")
    response = requests.post(f"{BASE_URL}/api/preferences/personalization-params", json={
        "workday_hours": 8,
    ...
    if response.status_code == 200:
        data = response.json()
        print(f"   [WARN] 部分校正: {data['message']}")
    ...
    if response.status_code == 200:
        data = response.json()
        print(f"   [WARN] 校验错误数: {len(data['validation_errors'])}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n[ERROR] 无法连接到后端服务！")
        print("请先启动后端服务: cd D:\\demo_plan\\backend && python main.py\n")
